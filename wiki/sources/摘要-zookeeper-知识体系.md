...
## 核心摘要
ZooKeeper 知识体系共 6 篇，涵盖核心概念（Znode/临时节点/永久节点/Watch 机制）、单机与集群安装部署、命令行操作（四字命令/ACL 权限）、原生 Java API（四种 CreateMode/会话重连）、三大应用场景（Master 选举/命名服务/分布式锁）和 Curator 客户端增强功能（重试策略/NodeCache/事务操作）。
