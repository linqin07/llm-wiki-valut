...
# 工具优先级 (Tool Priority)
**默认使用 CLI 工具**执行所有 vault 和知识库操作，以节约 token 消耗：
- **读取笔记**：优先使用 `obsidian read`，失败时回退到 Read 工具
- **创建笔记**：优先使用 `obsidian create`，失败时回退到 Write 工具
- **搜索内容**：优先使用 `obsidian search`，失败时回退到 Grep 工具
- **编辑笔记**：优先使用 `obsidian append`，失败时回退到 Edit 工具

此规则适用于所有 skill（ingest、query、lint 等）及直接的 vault 操作。
...
3. **内容分类**：
   ...
   - `/wiki/syntheses/`：存放复杂问题的综合回答或深度分析。
4. **强制双向链接**：
