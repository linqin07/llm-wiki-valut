...
## 实战捕获工作流 (Capture Workflow)
从日常 Claude Code 编码对话中提取有价值的问题解决经验：

1. **运行 `python tools/capture.py`**（或 `./tools/capture.sh`）
   - 交互式选择项目和对话，自动提取 user/assistant 消息
   - 过滤掉 tool 调用、系统消息、权限确认等噪音
   - 生成干净文章到 `raw/01-articles/cc-捕获-日期-主题.md`
2. **运行 `/ingest`** 将生成的文章编译入库

脚本位于 `tools/` 目录，通过 git 同步可在所有设备复用。
